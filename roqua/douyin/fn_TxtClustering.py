...
class Clustering:
    ...
    def fit(self, review_embeds, review_ids, k=8, limit=10, th=0.85):
    ...
    def get_review_embeds(self,ids):
        ...
        # vdb_name = "data/douyin_topic_milvus.db"
    ...
    def get_reviews(self, rdb, cluster_review_ids):
        ...
        cur = con.cursor()
        sql_t = "SELECT id, contents FROM reviews_id where id in {}"
        sql = sql_t.format(tuple(cluster_review_ids))
        res = cur.execute(sql)
        res = res.fetchall()
        cur.close()
        con.close()
...
if __name__ == '__main__':
    # review_embeds = np.random.randn(50, 10)
    # review_ids = np.arange(review_embeds.shape[0])
    # clusters, C = Clustering().fit(review_embeds, review_ids,k=2,th=0.5)
    # print(clusters)
    # for cluster, centroid in zip(clusters, C):
    #     e1 = np.sqrt(np.sum(centroid**2))
    #
    #     for index in cluster:
    #         vec = review_embeds[index]
    #         e2 = np.sqrt(np.sum(vec**2))
    #         val = np.dot(centroid, vec)/(e1*e2)
    #         print(val)
    #     print("----")

    rdb = param.rdb
    cluster_review_ids = [257,56,3, 12]
    model = Clustering()
    # res = model.get_reviews(rdb, cluster_review_ids)
    # print(res)

    cluster = model.run(rdb, cluster_review_ids, k=2)
    print(cluster)